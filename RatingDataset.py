...
class TestDataset(Dataset):
    ...
    def __init__(self, train_dataset, test_data_path, test_negative_path):
        ...
        self.load_test_ratings(test_data_path, test_negative_path)
        ...
        for i in range(len(self.test_ratings_list)):
            length = len(self.test_ratings_list[i][1:]) + len(
                self.test_neg_ratings_list[i][1:])

            if length > self.max_test_item_num:
                self.max_test_item_num = length

        print(
            "test ratings %d, test negative rating lists %d, maximum test item number for one rating %d"
            % (len(self.test_ratings_list), len(
                self.test_neg_ratings_list), self.max_test_item_num))

    def load_test_ratings(self, test_data_path, test_negative_path):
        self.test_ratings_list = []
        self.test_neg_ratings_list = []
        with open(test_data_path, "r") as input:
            for line in input.read().splitlines():
                ...
                true_ratings.extend(
                    [i for i in list(map(int, tokens[1].split(" ")))])
                # negative_ratings = [
                #     i for i in list(map(int, tokens[2].split(" ")))
                # ]

                self.test_ratings_list.append(true_ratings)
                # self.test_neg_ratings_list.append(negative_ratings)
        with open(test_negative_path, "r") as input:
            for line in input.read().splitlines():
                tokens = line.split(",")

                negative_ratings = [int(tokens[0])]
                negative_ratings.extend(
                    [i for i in list(map(int, tokens[1].split(" ")))])
    ...
    def __getitem__(self, idx):
        ...
        negative_id_num = len(self.test_neg_ratings_list[idx]) - 1
        real_test_item_size = len(self.test_ratings_list[idx][1:]
                                  ) + negative_id_num  # self.max_test_item_num
        ...
        test_item_ids[0:negative_id_num] = self.test_neg_ratings_list[idx][1:]
        test_item_ids[negative_id_num:
                      real_test_item_size] = self.test_ratings_list[idx][1:]
        assert self.test_ratings_list[idx][0] == self.test_neg_ratings_list[
            idx][0]
        ...
        for i in range(len(self.metapath_list)):
            ...
            metapath_input_list.append(
                np.zeros((self.max_test_item_num, self.metapath_list[i][2],
                          self.metapath_list[i][3], self.feature_size),
                         dtype=np.float32))

        k = 0
        # negative item ids
        for i in self.test_neg_ratings_list[idx][1:]:
            user_input[k] = u
            item_input[k] = i
            ...
            for metapath_idx in range(len(self.metapath_list)):
                ...
                if (u, i) in self.metapath_list[metapath_idx][1]:
                    for p_i in range(
                            len(self.metapath_list[metapath_idx][1][(u, i)])):
                        for p_j in range(
                                len(self.metapath_list[metapath_idx][1][(
                                    u, i)][p_i])):
                            type_id = self.metapath_list[metapath_idx][1][(
                                u, i)][p_i][p_j][0]
                            node_id = self.metapath_list[metapath_idx][1][(
                                u, i)][p_i][p_j][1]
                            node_type = self.id2type[type_id]
                            metapath_input_list[metapath_idx][k][p_i][
                                p_j] = self.features[node_type][node_id]
        ...
        data = [
            real_test_item_sizes, positive_item_indices,
            np.array(test_item_ids), user_input, item_input
        ]
...
class TrainDataset(Dataset):
    ...
    def __init__(self, train_data_path, metapath_file_paths, negative_num,
                 feature_file_dict):
    ...
    def __getitem__(self, idx):
        ...
        for metapath in self.metapath_list:
            ...
            metapath_input = np.zeros((self.negative_num + 1, metapath[2],
                                       metapath[3], self.feature_size),
                                      dtype=np.float32)
            ...
            if (u, i) in metapath[1]:
                for p_i in range(len(metapath[1][(u, i)])):
                    for p_j in range(len(metapath[1][(u, i)][p_i])):
                        ...
                        node_type = self.id2type[type_id]
                        metapath_input[counter][p_i][p_j] = self.features[
                            node_type][node_id]
        ...
        for t in range(self.negative_num):
            ...
            for list_index in range(len(metapath_input_list)):
                ...
                if (u, j) in self.metapath_list[list_index][1]:
                    for p_i in range(
                            len(self.metapath_list[list_index][1][(u, j)])):
                        for p_j in range(
                                len(self.metapath_list[list_index][1][(
                                    u, j)][p_i])):
                            type_id = self.metapath_list[list_index][1][(
                                u, j)][p_i][p_j][0]
                            node_id = self.metapath_list[list_index][1][(
                                u, j)][p_i][p_j][1]
                            node_type = self.id2type[type_id]
                            metapath_input_list[list_index][counter][p_i][
                                p_j] = self.features[node_type][node_id]
    ...
    def load_feature_as_map(self, feature_file_dict):
        ...
        for feature_type, feature_file_path in feature_file_dict.items():
            self.features[feature_type] = np.zeros(
                (self.node_sizes[feature_type] + 1, self.feature_size),
                dtype=np.float32)
            with open(feature_file_path) as input:
                for line in input.readlines():
                    ...
                    for j in range(len(arr[1:])):
                        self.features[feature_type][node_id][j] = float(arr[j +
                                                                            1])
        ...
        for metapath_file in metapath_files:
            ...
            with open(metapath_file) as input:
                for line in input.read().splitlines():
                    ...
                    hop_num = len(arr[2].split(" ")[0].split('-'))
            ...
            self.metapath_list.append(
                (metapath_file, path_dict, max_path_num, hop_num))
