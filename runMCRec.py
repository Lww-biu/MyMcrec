...
def read_all_metapath_files(metapath_files):
    path = root + "meta-path"
    filelist = os.listdir(path)
    metapath_files.extend(filelist)
...
if __name__ == "__main__":
    ...
    metapath_files = []
    read_all_metapath_files(metapath_files)
    feature_file_dict = {
        "u": "user_node_emb2.dat",
        "m": "item_node_emb2.dat",
        "g": "genre_node_emb2.dat",
    }

    train_data_file = "train.dat"
    test_data_file = "test.dat"
    test_negative_file = "negative.dat"

    for i in range(len(metapath_files)):
        metapath_files[i] = root + "meta-path/" + metapath_files[i]
    ...
    train_dataset = TrainDataset(root + train_data_file, metapath_files,
                                 negative_num, feature_file_dict)
    test_dataset = TestDataset(train_dataset, root + test_data_file,
                               root + test_negative_file)
    ...
    for i in range(len(train_dataset.metapath_list)):
        metapath_list_attributes.append((train_dataset.metapath_list[i][2],
                                         train_dataset.metapath_list[i][3]))
    ...
    results = evaluate(test_data_loader, model, test_dataset.max_test_item_num,
                       eval_processes_num)
    ...
    for i in range(len(topk)):
        k = topk[i]
        print(
            "Init: Item Pred: P@%d = %.4f, R@%d = %.4f, NDCG@%d = %.4f, MRR@%d = %.4f"
            % (k, results[i][0], k, results[i][1], k, results[i][2], k,
               results[i][3]))
    ...
    for epoch in range(epochs):
        ...
        for batch_data in train_data_loader:
            ...
            output = model(
                move_to_device(batch_data[0]),
                move_to_device(batch_data[1]),
                metapath_inputs,
            )
        ...
        print("Epoch %d Steps %d Loss %f [%.2f seconds]" %
              (epoch, total_step, cumulative_loss, t5 - t4))
        ...
        if (epoch + 1) % eval_frequency == 0:
            ...
            results = evaluate(
                test_data_loader,
                model,
                test_dataset.max_test_item_num,
                eval_processes_num,
            )
            ...
            for i in range(len(topk)):
                k = topk[i]
                print(
                    "Item Pred: P@%d = %.4f, R@%d = %.4f, NDCG@%d = %.4f, MRR@%d = %.4f"
                    % (
                        k,
                        results[i][0],
                        k,
    ...
    print(
        "Total Time %.2f seconds, Train Time %.2f seconds, Evaluation Time %.2f seconds"
        % (time.perf_counter() - start, train_total_time, eval_total_time))

    for i in range(len(topk)):
        k = topk[i]
        print(
            "Best Results for Item Pred: P@%d = %.4f, R@%d = %.4f, NDCG@%d = %.4f, MRR@%d = %.4f"
            % (
                k,
                max(pred_item_p_lists[i]),
                k,
                max(pred_item_r_lists[i]),
                k,
                max(pred_item_ndcg_lists[i]),
                k,
