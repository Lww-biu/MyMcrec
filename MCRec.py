...
class MetaPathEmbedding(nn.Module):
    ...
    def __init__(self, path_num, hop_num, feature_size, latent_dim):
        ...
        self.conv1D = nn.Conv1d(in_channels=self.feature_size,
                                out_channels=self.latent_dim,
                                kernel_size=kernel_size,
                                stride=1,
                                padding=0)
        self.lstm = nn.LSTM(self.latent_dim, self.latent_dim, num_layers=1)
    ...
    def forward(self, input):
        ...
        input = input.view(
            (-1, self.path_num, self.hop_num, self.feature_size))
...
class UIAttention(nn.Module):
    ...
    def __init__(self, latent_dim, att_size):
        ...
        self.dense = nn.Linear(in_features=latent_dim * 2,
...
class MCRec(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(metapath_list_attributes)):
            metapath_emb = MetaPathEmbedding(
                path_num=metapath_list_attributes[i][0],
                hop_num=metapath_list_attributes[i][1],
                feature_size=self.feature_size,
                latent_dim=self.latent_dim,
            )
            self.path_latent_vecs.append(metapath_emb)

        self.metapath_att = MetaPathAttention(
            att_size=self.att_size,
            latent_dim=self.latent_dim,
            metapath_type_num=len(metapath_list_attributes))
        self.user_att = UIAttention(latent_dim=self.latent_dim,
                                    att_size=self.att_size)
        self.item_att = UIAttention(latent_dim=self.latent_dim,
                                    att_size=self.att_size)
        ...
        dense_layer = nn.Linear(in_features=self.att_size * 3,
                                out_features=layer_size[0])
        self.layers.append(dense_layer)

        for i in range(1, len(layer_size)):
            dense_layer = nn.Linear(in_features=layer_size[i - 1],
    ...
    def forward(self, user_input, item_input, metapath_inputs):
        ...
        path_output = path_output.view(
            (-1, len(metapath_inputs), self.latent_dim))
