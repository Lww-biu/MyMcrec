...
def evaluate(test_data_loader, model, max_test_item_num, eval_processes_num):
    ...
    for _batch_data in test_data_loader:
        ...
        with torch.no_grad():
            ...
            _predictions = model(move_to_device(_batch_data[3]),
                                 move_to_device(_batch_data[4]),
                                 metapath_inputs)
        ...
        func = partial(eval_one_batch, _batch_data, _predictions)
        batch_results = pool.map(func, range(batch_size))
    ...
    for i in range(3):
        results.append((pred_item_ps[i], pred_item_rs[i], pred_item_ndcgs[i],
                        pred_item_mrrs[i]))

    return results


def eval_one_batch(_batch_data, _predictions, idx):

    map_item_score = {}
    # _batch_data[idx] = [real_test_item_size, positive_item_indices, test_item_ids, user_input, item_input, metapath_inputs]
    # print("idx"+str(idx))
    # print("batch_data"+str(_batch_data))
    # print(len(_batch_data[0]))
...
def getR(ranklist, gtItems):
    ...
    return (r_1 * 1.0 / len(gtItems), r_5 * 1.0 / len(gtItems),
            r_10 * 1.0 / len(gtItems))
